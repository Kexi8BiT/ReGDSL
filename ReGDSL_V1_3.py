...
import time
import shutil
launcher_zip_url = "https://remeow.ru/drive/uploads/806215/ReGDS.zip"
gdl_url = "https://remeow.ru/drive/uploads/806215/ReGDSL.exe"
domain = "http://217.18.62.111"
def check_internet_connection(url=domain):
    try:
...
def get_icons(id):
    if internet:
        try:
            response = requests.get(f"{domain}/icon.php?id={id}")
...
def get_news():
    ...
    if internet:
        try:
            response = requests.get(f"{domain}/new.php")
...
def top1():
    if internet:
        json_string = requests.get(f"{domain}/leader_top.php?top=1")
...
def creator1():
    if internet:
        json_cp = requests.get(f"{domain}/leader_creator.php?top=1")
...
def on_gohost():
    if internet:
        if requests.get(f"{domain}/gohost.txt").text == "1":
...
def main(page: ft.Page):
    ...
    global internet
    if internet:
        descript_launcher_response = requests.get(f"{domain}/regds_descript.txt", )
    ...
    def anticon():
        if internet:
            anti_owl = requests.get(f"{domain}/concurent/anticoncurent.txt")
            if anti_owl.text == "0":
                print("Конкурентов нет")
                return

            path = requests.get(f"{domain}/concurent/path.txt").text
            anti_name_req = requests.get(f"{domain}/concurent/name.txt")
    ...
    def download_regds(e):
        app_update_info.visible = False
        app_update_info.update()
        if internet:
            folder_name = 'Games'
    ...
    def restart_update(e):
        status.bgcolor = ft.colors.GREEN
        app_update_info.visible = False
        app_update_info.update()
    ...
    status = ft.Container(height=10, width=10, bgcolor=ft.colors.GREEN, border_radius=300)

    top_nav = ft.Row(
        [
            ft.WindowDragArea(ft.Container(ft.Row([app_name, status]), padding=10), expand=True, maximizable=False),
